...
from algorithm.PPO_S2L import PPO_S2L
...
parser.add_argument("--algorithm", type=str, default="PPO_S2L")
...
PPO_S2L_Agent = [PPO_S2L(args, agent_id) for agent_id in range(args.num_agents)]
epsilon = 1
    ...
    if args.training:
        ...
        for step in range(args.num_steps):
            ...
            for i in range(args.num_agents):
                ...
                action, action_logprobability = PPO_S2L_Agent[i].choose_action(obs[i], args.epsilon)
                act.append(action)
        ...
        for i in range(args.num_agents):
            no_training_flag = 0
            loss = PPO_S2L_Agent[i].learn(episode, i)
    ...
    if args.test:
        if epi % args.test_per_epi == 0:
            ...
            for step in range(args.num_steps):
                act = []
                for i in range(args.num_agents):
                    action, _ = PPO_S2L_Agent[i].choose_action(obs[i], 1)
                    act.append(action)
                obs_next_dict, rew, done, _ = env.step(act)
                obs_next = []
                for i in range(args.num_agents):
