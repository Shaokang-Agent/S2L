...
from algorithm.DQN_RND import DQN_RND
...
parser.add_argument("--algorithm", type=str, default="DQN_RND")
...
DQN_RND_Agent = [DQN_RND(args, agent_id) for agent_id in range(args.num_agents)]
    ...
    if args.training:
        ...
        for step in range(args.num_steps):
            ...
            for i in range(args.num_agents):
                ...
                act.append(DQN_RND_Agent[i].choose_action(obs[i], args.epsilon, "training"))
        ...
        if args.batch_size < Repaly_Buffer.__len__() and epi % 3 == 0:
            ...
            for i in range(args.num_agents):
                loss = DQN_RND_Agent[i].learn(mini_batch, i)
    ...
    if args.test:
        if epi % args.test_per_epi == 0:
            ...
            for step in range(args.num_steps):
                act = []
                for i in range(args.num_agents):
                    act.append(DQN_RND_Agent[i].choose_action(obs[i], 1, "test"))
                obs_next_dict, rew, done, _ = env.step(act)
                obs_next = []
                for i in range(args.num_agents):
                    obs_next.append(obs_next_dict[i]['pov']/255)
