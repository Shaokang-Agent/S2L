...
class DQN_MAXEQ():
        ...
        if args.load:
            ...
            if os.path.exists(model_root_path):
                ...
                self.eval_net.load_state_dict(
                    torch.load(model_root_path + '/' + str(num) + '_Q_Net.pkl', map_location='cpu'))
                self.input_net.load_state_dict(
                    torch.load(model_root_path + '/' + str(num) + '_input_net.pkl', map_location='cpu'))
                print('Agent {} successfully loaded DQN_MAXEQ-{}'.format(self.agent_id, num))
    ...
    def learn(self, episode_data, agent_id):
        ...
        if self.args.double_dqn:
            q_target = batch_reward + self.args.gamma * q_next.gather(2, self.eval_net(
        ...
        action_prob_learning = F.softmax(q_eval / 0.1, dim=-1)
        dist = Categorical(action_prob_learning)
        loss = self.loss_function(q_eval, q_target) - 0.05*dist.entropy().mean()
