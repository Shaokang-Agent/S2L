...
class PPO_S2L():
    def __init__(self, args, agent_id):
        ...
        if args.load:
            ...
            if os.path.exists(model_root_path):
                ...
                if args.self_learning:
                    ...
                    self.explore_critic.load_state_dict(torch.load(model_root_path + '/' + str(num) + '_explore_critic.pkl', map_location='cpu'))
                if args.rnd:
        ...
        if self.args.self_learning:
            ...
            self.training_parameters.append({'params': self.explore_critic.parameters(), 'lr': self.args.lr})
    ...
    def learn(self, episode_data, agent_id):
        ...
        with torch.no_grad():
            ...
            if self.args.self_learning:
                ...
                #shaping_reward = torch.clamp(shaping_reward/10, -0.25, 0.25)
                if self.learn_step_counter % 10 == 0:
                    print("env_reward:{}, self_learning_reward:{}".format(batch_reward[0].mean(), shaping_reward[0].mean()))
                reward += self.args.shaping_rewards_alpha/2 * shaping_reward
        ...
        if self.args.self_learning and self.learn_step_counter % 1 == 0:
            ...
            torch.nn.utils.clip_grad_norm_(self.input_net.parameters(), self.args.grad_clip/2)
            torch.nn.utils.clip_grad_norm_(self.explore_reward_net.parameters(), self.args.grad_clip/2)
            self.optimizer.step()
            ...
            torch.nn.utils.clip_grad_norm_(self.explore_critic.parameters(), self.args.grad_clip)
    ...
    def save_model(self,num):
        ...
        if self.args.self_learning:
            ...
            torch.save(self.explore_critic.state_dict(), model_path + '/' + str(num) + '_explore_critic.pkl')
        if self.args.rnd:
