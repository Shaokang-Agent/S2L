...
from algorithm.PPO_MAXEQ import PPO_MAXEQ
...
parser.add_argument("--algorithm", type=str, default="PPO_MAXEQ")
...
PPO_MAXEQ_Agent = [PPO_MAXEQ(args, agent_id) for agent_id in range(args.num_agents)]
epsilon = 1
    ...
    if args.training:
        ...
        for step in range(args.num_steps):
            ...
            for i in range(args.num_agents):
                ...
                action, action_logprobability = PPO_MAXEQ_Agent[i].choose_action(obs[i], args.epsilon)
                act.append(action)
        ...
        for i in range(args.num_agents):
            loss = PPO_MAXEQ_Agent[i].learn(episode, i)
    ...
    if args.test:
        if epi % args.test_per_epi == 0:
            ...
            for step in range(args.num_steps):
                act = []
                for i in range(args.num_agents):
                    action, _ = PPO_MAXEQ_Agent[i].choose_action(obs[i], 1)
                    act.append(action)
                obs_next_dict, rew, done, _ = env.step(act)
                obs_next = []
                for i in range(args.num_agents):
