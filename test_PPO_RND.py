...
from algorithm.PPO_RND import PPO_RND
...
parser.add_argument("--algorithm", type=str, default="PPO_RND")
...
parser.add_argument("--shaping_rewards_alpha", type=float, default=0.4)
...
PPO_RND_Agent = [PPO_RND(args, agent_id) for agent_id in range(args.num_agents)]
epsilon = 1
...
for epi in tqdm(range(args.num_episodes)):
    args.shaping_rewards_alpha = max([0.01, args.shaping_rewards_alpha - 0.00005])
    if args.training:
        for i in range(args.num_agents):
            PPO_RND_Agent[i].init_hidden()
        ...
        for step in range(args.num_steps):
            ...
            for i in range(args.num_agents):
                ...
                action, action_logprobability = PPO_RND_Agent[i].choose_action(obs[i], args.epsilon)
                act.append(action)
        ...
        for i in range(args.num_agents):
            loss = PPO_RND_Agent[i].learn(episode, i)
    ...
    if args.test:
        if epi % args.test_per_epi == 0:
            ...
            for step in range(args.num_steps):
                act = []
                for i in range(args.num_agents):
                    action, _ = PPO_RND_Agent[i].choose_action(obs[i], 1)
                    act.append(action)
                obs_next_dict, rew, done, _ = env.step(act)
                obs_next = []
                for i in range(args.num_agents):
